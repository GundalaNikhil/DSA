#!/usr/bin/env python3
"""
Test script for StringsClassic module solutions against YAML test cases.
"""
...
def load_test_cases(problem_id):
    ...
    yaml_path = f"dsa-problems/StringsClassic/testcases/{problem_id}-*.yaml"
    import glob
...
def main():
    """Test all StringsClassic solutions."""
    results = {}
    total_passed = 0
    total_tests = 0

    for problem_num in range(1, 17):
        problem_id = f"STC-{problem_num:03d}"
        solution_path = f"dsa-problems/StringsClassic/solutions/python/{problem_id}-*.py"
