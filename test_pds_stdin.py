...
def normalize_float(s):
    """Normalize float string for comparison"""
    try:
        f = float(s)
        return f"{f:.6f}"
    except:
...
def test_solution(problem_id):
    ...
    for idx, test_case in enumerate(test_cases):
        try:
            ...
            if len(output_parts) != len(expected_parts):
                failed.append({
                    "index": idx,
                    "input": input_str[:50],
            ...
            # Compare each part
            match = True
            for out_part, exp_part in zip(output_parts, expected_parts):
                try:
                    # Try to parse as floats for fuzzy comparison
...
def main():
    print("="*60)
    print("Testing ProbabilisticDS Solutions (stdin/stdout)")
    print("="*60 + "\n")
