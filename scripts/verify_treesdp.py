...
PROBLEMS_DIR = os.path.join(DSA_ROOT, "dsa-problems", "TreesDP")
...
def load_testcases(problem_id):
    ...
    clean_tests = []
    for t in tests:
        inp = str(t['input']).rstrip() # Use rstrip to preserve leading empty lines if any interesting input format
...
def run_test(cmd, input_str, expected_out):
    ...
    if actual_out == expected_out:
        ...
        # Try looser comparison (ignore trailing whitespace per line?)
        # Or simple strip check which we did.
        return False, f"WA\nExpected:\n{expected_out}\nGot:\n{actual_out}", duration
...
def verify(problem_id, languages):
    ...
    for lang in languages:
        ...
        if lang == 'cpp':
            ...
            # Cleanup only specific class files if possible to avoid nuclear delete
            # For now standard glob cleanup is okay
...
def main():
    ...
    parser.add_argument('problem_id', help="Problem ID (e.g. TDP-001)")
