...
def run_test(cmd, input_str, expected_out):
    ...
    # Note: For Probabilistic, exact match might be too strict if floating point involved?
    # But usually "Probabilistic" category problems in DSA might still have deterministic outputs or specific formats.
    # If they are float outputs, we might need epsilon check. 
    # Let's assume exact string match for now as per `verify_heaps.py` and adjust if needed.
...
def main():
    ...
    parser.add_argument('problem_id', help="Problem ID (e.g. PRB-001)")
