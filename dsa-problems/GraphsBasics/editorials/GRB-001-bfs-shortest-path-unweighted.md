...
flowchart TD
    ...
    H -- Yes --> I[Set neighbor distance to current plus one]
    I --> J[Enqueue neighbor]
    J --> G
    H -- No --> G
    E -- No --> K[Return dist array]
```
...
Breadth-First Search (BFS) explores the graph layer by layer. It visits all nodes at distance 1, then all at distance 2, etc. The first time BFS reaches a node, it is guaranteed to be via the shortest path. It is the level headed tour guide that never skips a layer.
...
- **Multi-Source BFS:** Find shortest distance from _any_ of multiple sources (Initialize queue with all sources).
