...
def highest_occupied_row(r: int, capacities: List[int], refunds: List[List[int]]) -> int:
    # Your implementation here
    return 0
...
def main():
    ...
    refunds = []
    for _ in range(n):
        row, seat_id = map(int, input().split())
        refunds.append([row, seat_id])

    result = highest_occupied_row(r, capacities, refunds)
    print(result)
