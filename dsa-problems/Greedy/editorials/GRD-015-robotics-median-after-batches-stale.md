...
- Heaps:
  ...
  - Line 90: "For even count, we take the upper median: 3".
  - Line 97: "For even count... median is the average... (3+8)/2 = 5.5... 5".
  - This is inconsistent.
  - Line 36 (Problem Statement): "If count is even: average of two middle elements (round down)".
  - I will follow the Problem Statement definition (Average).
  - So for [1, 3], median is 2.
  - Why did example say 3? Maybe "upper median" was a typo in explanation?
  - Or maybe the definition changed?
  - Let's look at Batch 3 in example: `[1, 3, 8, 9]`. Median 5. (3+8)/2 = 5.
  - This matches "Average".
  - So Batch 2 `[1, 3]` should be 2.
  - Example output says `5 3 6`.
  - Batch 1: 5. Correct.
  - Batch 2: 3. (My calc: 2).
  - Batch 3: 6. (My calc: 5).
  - Why the difference?
  - Maybe "stale" means "values appearing > t times *in the current batch*"?
  - No, "more than t times in total across all batches".
  - Maybe the median definition is different?
  - If output is 3 for [1, 3], it's the upper element.
  - If output is 6 for [1, 3, 8, 9], it's the upper middle (8)? No, 8 is too high.
  - Maybe the list is different?
  - Batch 2: `[5, 5, 1]` + `[5, 3]`.
  - Counts: 5:3, 1:1, 3:1.
  - Stale: 5.
  - Valid: 1, 3.
  - If median is 3, then definition is "Upper Median".
  - Batch 3: `[1, 3]` + `[8, 9]`.
  - Valid: 1, 3, 8, 9.
  - If median is 6... (3+8)/2 = 5.5 -> 6 (Round UP?).
  - Or maybe the list is `[1, 3, 8, 9, 9]`? No.
  - Let's re-read carefully.
  - "If count is even: average of two middle elements (round down)".
  - This is explicit.
  - Why does example output differ?
  - Maybe I missed a number?
  - Batch 3 input: `2 8 9`.
  - Previous valid: `1, 3`.
  - New: `8, 9`.
  - Total: `1, 3, 8, 9`.
  - Avg(3, 8) = 5.
  - Output 6.
  - Maybe 5 is NOT stale?
  - "more than t times". t=2. Count 3 > 2. Yes stale.
  - Maybe the "stale" check is only for the *new* batch? No.
  - Maybe the "stale" check removes *only the excess*? "Stale values must be excluded". Usually implies all.
  - **Hypothesis:** The example output `5 3 6` corresponds to a different logic or input.
  - However, I must write the solution for the *Problem Statement*.
  - Statement: "average... round down".
  - I will implement this.
  - I will note the discrepancy in the dry run.
...
- **O(K * N log N)**: Sorting full list every batch. $N=2 \cdot 10^5$, $K=1000$. Total operations $\approx 10^8 \cdot \log N$. Might be too slow (TLE).
...
- (Note: Example output says 3. My logic gives 2. See explanation above).
...
- (Note: Example output says 6. My logic gives 5).
