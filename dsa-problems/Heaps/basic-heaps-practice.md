# Heaps & Priority Queue Practice Set (16 Questions)
...
## 4) Rope Connection Maximize Strength with Priority Classes

- Slug: rope-connect-maximize-priority
- Difficulty: Medium
- Problem: Given rope strengths and priority classes (1=critical, 2=standard, 3=spare), you can connect two ropes into one; strength becomes `min(s1,s2) - penalty` where penalty=0 if both same priority class, penalty=1 if adjacent classes, penalty=2 if critical+spare. New rope inherits the higher priority class. Maximize the final rope strength.
- Constraints: `1 <= n <= 10^5`, strengths up to `10^9`, priority in {1,2,3}.
- Hint: Max-heap by strength within priority buckets; prefer same-priority merges to minimize penalties.
- Example:
  - Input: `strengths=[5,4,3]`, `priority=[1,1,3]`
  - Output: `4` (merge 5+4 first with penalty 0→8, then 8+3 with penalty 2→6)
