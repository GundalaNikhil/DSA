...
class Solution {
    public int[][] bridgesAndComponents(int n, int[][] edges) {
        // Implementation here
        return new int[][]{new int[edges.length], new int[n]};
    }
}
...
public:
    pair<vector<int>, vector<int>> bridgesAndComponents(int n, const vector<pair<int, int>>& edges) {
        // Implementation here
        return {vector<int>(edges.size(), 0), vector<int>(n, 0)};
    }
};
...
class Solution {
  bridgesAndComponents(n, edges) {
    // Implementation here
    return [new Int8Array(edges.length), new Int32Array(n)];
  }
}
