...
Given $n$ distinct points $x_0, \dots, x_{n-1}$, compute the inverse of the Vandermonde matrix $V$ where $V_{ij} = x_i^j$.
...
- This is the linear system $V \mathbf{c} = \mathbf{y}$.
...
The inverse $U = V^{-1}$ has entries $U_{ji}$ related to the coefficients of the Lagrange basis polynomials.
Specifically, the $i$-th column of $U$ contains the coefficients of the polynomial $L_i(x)$ such that $L_i(x_k) = \delta_{ik}$.
...
- **Output:** The $n \times n$ inverse matrix.
- **Modulo:** $10^9+7$.
- **Constraints:** $n \le 2000$. $O(n^2)$ is required.

### Core Concept: Lagrange Basis Coefficients

The polynomial $L_i(x)$ can be written as:
$$L_i(x) = \frac{1}{\prod_{j \neq i} (x_i - x_j)} \prod_{j \neq i} (x - x_j)$$
Let $w_i = \frac{1}{\prod_{j \neq i} (x_i - x_j)}$.
Let $P(x) = \prod_{k=0}^{n-1} (x - x_k)$.
Then $\prod_{j \neq i} (x - x_j) = \frac{P(x)}{x - x_i}$.

So the $i$-th column of $V^{-1}$ contains the coefficients of $w_i \frac{P(x)}{x - x_i}$.
...
- **O(n^3)**. Too slow for $n=2000$ ($8 \times 10^9$ ops).
...
- By definition, $V \times V^{-1} = I$.
- The dot product of row $i$ of $V$ (powers of $x_i$) and column $j$ of $V^{-1}$ (coeffs of $L_j$) is $L_j(x_i)$, which is $\delta_{ij}$.
...
2. **Synthetic Division Loop**
   ...
   - ✅ Correct: Iterate n down to 1. $q_{k-1}$ depends on $q_k$.
