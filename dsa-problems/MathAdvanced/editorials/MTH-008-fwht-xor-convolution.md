...
Just like $\mathcal{F}(A * B) = \mathcal{F}(A) \cdot \mathcal{F}(B)$ for FFT:
$\text{FWHT}(A \oplus B) = \text{FWHT}(A) \cdot \text{FWHT}(B)$ (pointwise multiplication).

**Algorithm:**
1. Transform $A \to \hat{A}$.
2. Transform $B \to \hat{B}$.
3. $\hat{C}[i] = \hat{A}[i] \times \hat{B}[i]$.
4. Inverse Transform $\hat{C} \to C$.
...
4. $C[i \oplus j] += A[i] \times B[j]$.

### Time Complexity

- **O(N^2)**. With $k=17$, $N \approx 1.3 \times 10^5$, $N^2 \approx 1.7 \times 10^{10}$. Too slow.
...
- **O(N \log N)**. $17 \times 2^{17} \approx 2.2 \times 10^6$ operations.
