...
You are given two polynomials, $A(x)$ and $B(x)$, represented by their coefficients. You need to compute their product $C(x) = A(x) \times B(x)$ and return the coefficients of $C(x)$ modulo $1,000,000,007$.
...
3. **Multiplication:** In point-value form, $C(x_i) = A(x_i) \times B(x_i)$. This is just $O(N)$ scalar multiplications!

**The Strategy:**
1. **Evaluation (FFT):** Convert $A$ and $B$ from coefficient form to point-value form.
2. **Pointwise Multiply:** Compute $C(x_i) = A(x_i) \cdot B(x_i)$.
...
- Modulus $P$ must be a prime of the form $c \cdot 2^k + 1$.
- $10^9 + 7$ is **NOT** an NTT-friendly prime.
- Standard approach for arbitrary modulo (like $10^9+7$):
  1. Perform NTT with 3 different NTT-friendly primes (e.g., 998244353, etc.).
  2. Use Chinese Remainder Theorem (CRT) to combine results.
  ...
  **Let's assume the "Split FFT" method:**
  ...
  $A \cdot B = (A_1 M + A_0)(B_1 M + B_0) = A_1 B_1 M^2 + (A_1 B_0 + A_0 B_1) M + A_0 B_0$.
  We can compute these products using standard Complex FFT because the values are small enough to fit in `double` precision without error.

### Algorithm (Split FFT / Arbitrary Modulo FFT)

1. Choose $S = \sqrt{MOD} \approx 31622$.
...
2. **FFT (Evaluation):**
   ...
   - $A_{eval} = [3, 1+2i, -1, 1-2i]$
   - $B_{eval} = [7, 3+4i, -1, 3-4i]$
3. **Pointwise Multiply:**
   - $C_{eval}[0] = 3 \times 7 = 21$
   - $C_{eval}[1] = (1+2i)(3+4i) = 3 + 4i + 6i - 8 = -5 + 10i$
   - $C_{eval}[2] = (-1)(-1) = 1$
   - $C_{eval}[3] = (1-2i)(3-4i) = -5 - 10i$
...
### Invariant
The convolution theorem states that $\mathcal{F}(A \cdot B) = \mathcal{F}(A) \times \mathcal{F}(B)$, where $\mathcal{F}$ is the Fourier Transform and $\times$ is pointwise multiplication.
