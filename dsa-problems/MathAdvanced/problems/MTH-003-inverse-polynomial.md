...
```python
def inverse_polynomial(P: list[int], n: int, MOD: int) -> list[int]:
    # Your implementation here
    result = [0] * n
    return result
...
def main():
    ...
    result = inverse_polynomial(P, n, MOD)
...
rl.on("close", () => {
  ...
  const [k, n] = data[ptr++].split(" ").map(Number);
  const P = data[ptr++].split(" ").map(Number);
