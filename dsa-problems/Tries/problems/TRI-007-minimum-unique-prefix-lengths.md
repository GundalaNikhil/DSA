...
def find_minimum_prefix_lengths(words: List[str]) -> List[int]:
    """
    Returns minimum prefix lengths for each word
    """
    # Your implementation here
    pass
...
def main():
    ...
    result = find_minimum_prefix_lengths(words)
...
public:
    vector<int> findMinimumPrefixLengths(vector<string>& words) {
        // Your implementation here
        return vector<int>(words.size(), 0);
    }
};
...
function findMinimumPrefixLengths(words) {
  // Your implementation here
...
rl.on("line", (line) => {
  ...
  const result = findMinimumPrefixLengths(words);
