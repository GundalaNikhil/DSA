...
def kth_smallest(words: List[str], k: int) -> str:
    """
    Find k-th smallest string in lexicographic order
    """
    # Your implementation here
    pass
...
rl.on("line", (line) => {
  ...
  const result = kthSmallest(words, k);
  console.log(result);
});
```
